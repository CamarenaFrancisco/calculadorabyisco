...
;(function () {
  ...
  core.Utils.subFunctions = function (symbol, map) {
    ...
    symbol.each(function (x) {
      if (x.group === ES_FUNCIÓN || x.previousGroup === ES_FUNCIÓN) {
  ...
  var __ = (core.Algebra = {
    ...
    proots: function (symbol, decp) {
      ...
      if (symbol instanceof Symbol && symbol.isPoly()) {
        ...
        var variable = keys(symbol.symbols)
            .sort()
            .pop(),
          sym =
    ...
    Factor: {
      ...
      _factor: function (symbol, factors) {
        ...
        if (symbol.group === ES_FUNCIÓN && symbol.fname !== 'sqrt') {
        ...
        try {
          ...
          if (isInt(p) && !(p.lessThan(0) && symbol.group === ES_FUNCIÓN)) {
            var sign = p.sign()
            symbol.toLinear()
    ...
    gcd: function () {
      ...
      for (var i = 0; i < args.length; i++) {
        if (args[i].group === ES_FUNCIÓN && args[i].fname === 'gcd') {
    ...
    gcd_: function (a, b) {
      if (a.group === ES_FUNCIÓN || a.group === P)
        a = core.Utils.block('PARSE2NUMBER', function () {
          return _.parse(a)
        })

      if (b.group === ES_FUNCIÓN)
    ...
    Simplify: {
      ...
      simplificacion_trigonometrica: function (symbol) {
        if (
          symbol.containsFunction(['cos', 'sin', 'tan', 'sec', 'csc', 'cot'])
        ) {
          symbol = symbol.clone()
          ...
          if (symbol.group === ES_SUMA) {
            ...
            if (retorno.group === ES_PRODUCTO) {
              var t = new Symbol(1)
              retorno.each(function (x) {
                x = identidadesSenoCoseno(
                  'tan',
                  '({0})*sin({1})^({2})/cos({1})^({2})',
                  x
                )
                ...
                x = identidadesSenoCoseno('csc', '({0})*(1/cos({1})^({2}))', x)
                x = identidadesSenoCoseno('sec', '({0})*(1/sin({1})^({2}))', x)

                function identidadesSenoCoseno (fname, identidad, retorno) {
                  if (retorno.fname === fname) {
                    return _.parse(
                      core.Utils.format(
                        identidad,
                        retorno.multiplier,
                        __.Simplify.simplify(retorno.args[0]),
                        retorno.power
                      )
                    )
                  }
            ...
            var n = retorno.getNum()
            var d = retorno.getDenom()

            //identidad tangente
            if (n.fname === 'sin' && d.fname === 'cos') {
              if (n.args[0].equals(d.args[0]) && n.power.equals(d.power)) {
                retorno = _.parse(
                  core.Utils.format(
                    '({0})*({1})*tan({2})^({3})',
                    d.multiplier,
          ...
          retorno = identidadesInversas('sen', 'sec', retorno)
          retorno = identidadesInversas('cos', 'csc', retorno)

          function identidadesInversas (fname, inversa, simbolo) {
            if (
              simbolo.fname === fname &&
              simbolo.group == ES_FUNCIÓN &&
              simbolo.power.lessThan(0)
            ) {
              return _.parse(
                core.Utils.format(
                  `({0})*${inversa}({1})^({2})`,
      ...
      patternSub: function (symbol) {
        ...
        var collect = function (sym) {
          ...
          sym.each(function (x) {
            ...
            if (!x.symbols && x.group !== ES_FUNCIÓN) {
              return
            }
