...
(function () {
  ...
  function Equation(lhs, rhs) {
    if (
      ...
      rhs.equals(core.Settings.IMAGINARY)
    )
      throw new core.exceptions.NerdamerValueError(
        lhs.toString() + " does not equal " + rhs.toString()
      );
  ...
  var __ = (core.Solve = {
    ...
    solveCircle: function (eqns, vars) {
      ...
      if (
        ...
        var y_points = solve(
          _.parse(b, knownVariable(x, solve(_.parse(a), x)[0])),
          y
        ).map(function (x) {
          return x.toString();
        });
        ...
        if (y_points[1]) {
          x_points.push(
            solve(_.parse(a, knownVariable(y, y_points[1])))[0].toString()
          );
        }
    ...
    solveNonLinearSystem: function (eqns, tries, start) {
      ...
      start =
        typeof start === "undefined" ? core.Settings.NON_LINEAR_START : start;
    ...
    solveSystemBySubstitution: function (eqns) {
      ...
      if (
        eqns.length === 2 &&
        vars_a.length === 2 &&
        core.Utils.arrayEqual(vars_a, vars_b)
      ) {
    ...
    solveSystem: function (eqns, var_array) {
      ...
      if (typeof var_array === "undefined") {
        ...
        if (!_A.allLinear(eqns)) {
            console.log("se definio que el sistema no es lineal");
          try {
            console.log("se resuelve el sistema por opcion 1")
            return __.solveNonLinearSystem(eqns);
          } catch (e) {
            if (e instanceof core.exceptions.DivisionByZero) {
                console.log("se resuelve el sistema por opcion 2")
              return __.solveSystemBySubstitution(eqns);
            }
            console.log("hubo un error en el intento 1")
          }
        }
      ...
      if (vars.length !== eqns.length) {
        throw new core.exceptions.SolveError(
          "la cantidad de variables y de ecuaciones no corresponden para que haya solución"
        );
      }
      if (m.determinant().equals(0)) {
        throw new core.exceptions.SolveError(
          "El determinante es 0, entonces el sistema no tiene solución"
        );
      }
    ...
    bisection: function (point, f) {
      ...
      if (Math.sign(f(left)) !== Math.sign(f(right))) {
        ...
        if (
          !isNaN(x_point) &&
          Math.abs(x_point) <= core.Settings.BI_SECTION_EPSILON
        ) {
    ...
    rewrite: function (rhs, lhs, for_variable) {
      ...
      if (rhs.isComposite() && rhs.isLinear()) {
        ...
        if (sqrts.length === 1) {
          ...
          lhs = _.expand(
            _.pow(_.subtract(lhs, core.Utils.arraySum(rem)), new Symbol(2))
          );
          //square both sides
  ...
  var solve = function (eqns, solve_for, solutions, depth, fn) {
    ...
    if (depth++ > Settings.MAX_SOLVE_DEPTH) {
      console.log("no se pudo determinar una solución concreta");
      return solutions;
    }
    ...
    var eq = (
        core.Utils.isSymbol(eqns) ? eqns : __.toLHS(eqns, false)
      ).getNum(),
    ...
    __.inverseFunctionSolve = function (name, lhs, rhs) {
      ...
      if (x.group === S) {
        return _.divide(
          _.symfunction(name, [_.divide(rhs, _.parse(lhs.multiplier))]),
          parts[0]
        );
      }
    };
    ...
    if (numvars === 1) {
      if (eq.isPoly(true)) {
        ...
        try {
          ...
          var points = core.Utils.arrayUnique(
            points1.concat(points2).concat(points3)
          ).sort(function (a, b) {
            return a - b;
          });
